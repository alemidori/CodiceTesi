...
def get_specific_words(Ldawords, tokenlist):
    ...
    total_f = list(create_nyt_dataset.term_frequency.aggregate([{'$group': {'_id': None, 'count': {'$sum': 1}}}]))[0]['count']
    ...
    for word in Ldawords:
        ...
        if word in term_freqs.keys():
            ...
            for k in create_nyt_dataset.term_frequency.find({'term': word}, {'_id':0,'frequency':1}):
